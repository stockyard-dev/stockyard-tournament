...
func DefaultLimits() Limits {
	...
	claims := validateLicenseKey(key, "tournament")
	if claims == nil {
	...
	if claims.Tier != "individual" && claims.Tier != "*" {
		...
		for _, t := range claims.Tools {
			if t == "tournament" || t == "*" { found = true; break }
		}
		if !found {
			log.Printf("[license] Tool tournament not in licensed tools")
			return NoLicense()
		}
	}
...
func (s *Server) tierHandler(w http.ResponseWriter, r *http.Request) {
	...
	m["upgrade_url"] = "https://stockyard.dev/tournament/"
	wj(w, 200, m)
}
