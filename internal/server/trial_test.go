...
// Tests for the additive trial-gate middleware on tournament. Booking
...
//   7. End-to-end: tier=="none" → POST /api/tournaments returns 402
//   8. End-to-end: tier=="paid" → POST /api/tournaments returns 200
//   9. End-to-end: tier=="none" → GET /api/tournaments still works (read-through)
...
// tournament's existing limits.go doesn't expose a hook for that.

func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "tournament-trial-test-*")
	if err != nil {
...
func newTestServer(t *testing.T, tier string) *Server {
	t.Helper()
	dir, err := os.MkdirTemp("", "tournament-trial-srv-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(dir) })
...
func TestShouldBlockWrite_NoneTier(t *testing.T) {
	...
	for _, c := range cases {
		req := httptest.NewRequest(c.method, "/api/tournaments", nil)
...
func TestShouldBlockWrite_ExpiredTier(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/tournaments", nil)
	if !s.shouldBlockWrite(req) {
		t.Error("expired tier should block POST writes")
	}
	req = httptest.NewRequest("GET", "/api/tournaments", nil)
	if s.shouldBlockWrite(req) {
...
func TestShouldBlockWrite_PaidTier(t *testing.T) {
	...
	for _, m := range []string{"POST", "PUT", "DELETE", "PATCH", "GET"} {
		req := httptest.NewRequest(m, "/api/tournaments", nil)
		if s.shouldBlockWrite(req) {
...
func TestEndToEnd_NoneTierBlocksWrite(t *testing.T) {
	s := newTestServer(t, "none")
	body := bytes.NewBufferString(`{"name":"Spring Cup"}`)
	req := httptest.NewRequest("POST", "/api/tournaments", body)
...
func TestPersistLicense_WritesFile(t *testing.T) {
	dir, err := os.MkdirTemp("", "tournament-persist-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
...
func TestLoadLicenseFromDisk_RoundTrip(t *testing.T) {
	dir, err := os.MkdirTemp("", "tournament-load-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
...
func TestLoadLicenseFromDisk_MissingReturnsEmpty(t *testing.T) {
	dir, err := os.MkdirTemp("", "tournament-missing-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
