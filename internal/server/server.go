...
func (s *Server) activateLicense(w http.ResponseWriter, r *http.Request) {
	...
	if err := PersistLicense(s.dataDir, key); err != nil {
		log.Printf("tournament: license persist failed: %v", err)
	...
	s.limMu.Unlock()
	log.Printf("tournament: license activated via dashboard, persisted to %s/%s, tier=%s", s.dataDir, licenseFilename, newTier)
...
func (s *Server) listTournaments(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"tournaments": oe(s.db.SearchTournaments(q, filters))})
		return
	}
...
func (s *Server) delTournaments(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	s.db.DeleteTournaments(id)
	s.db.DeleteExtras("tournaments", id)
...
func (s *Server) listParticipants(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"participants": oe(s.db.SearchParticipants(q, filters))})
		return
	}
...
func (s *Server) createParticipants(w http.ResponseWriter, r *http.Request) {
	...
	json.NewDecoder(r.Body).Decode(&e)
	if e.TournamentId == "" {
		we(w, 400, "tournament_id required")
		return
	}
	if e.Name == "" {
		we(w, 400, "name required")
...
func (s *Server) updateParticipants(w http.ResponseWriter, r *http.Request) {
	...
	if patch.TournamentId == "" {
		patch.TournamentId = existing.TournamentId
	}
	if patch.Name == "" {
		patch.Name = existing.Name
	}
...
func (s *Server) delParticipants(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	s.db.DeleteParticipants(id)
	s.db.DeleteExtras("participants", id)
	wj(w, 200, map[string]string{"deleted": "ok"})
}
...
func (s *Server) listMatches(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"matches": oe(s.db.SearchMatches(q, filters))})
		return
	}
...
func (s *Server) updateMatches(w http.ResponseWriter, r *http.Request) {
	...
	if patch.Player1 == "" {
		patch.Player1 = existing.Player1
	}
	if patch.Player2 == "" {
		patch.Player2 = existing.Player2
	}
	if patch.Score1 == "" {
		patch.Score1 = existing.Score1
	}
	if patch.Score2 == "" {
		patch.Score2 = existing.Score2
	}
	if patch.Winner == "" {
...
func (s *Server) delMatches(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	s.db.DeleteMatches(id)
	s.db.DeleteExtras("matches", id)
	wj(w, 200, map[string]string{"deleted": "ok"})
}
