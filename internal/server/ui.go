...
const dashHTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Tournament</title>
...
<div class="hdr"><h1><span>&#9670;</span> TOURNAMENT</h1><button class="btn btn-p" id="add-btn">+ Add</button></div>
<div class="main"><div class="tabs" id="tabs"><div class="tab active" onclick="switchTab('tournaments')">Tournaments</div><div class="tab" onclick="switchTab('participants')">Participants</div><div class="tab" onclick="switchTab('matches')">Matches</div></div><div class="tab-content active" id="tab-tournaments">
<div class="stats" id="stats-tournaments"></div>
<div class="toolbar">
<input class="search" id="search-tournaments" placeholder="Search..." oninput="renderRes('tournaments')"><select class="filter-sel" id="filter-tournaments-format" onchange="renderRes('tournaments')"><option value="">All Format</option><option value="Single Elimination">Single Elimination</option><option value="Double Elimination">Double Elimination</option><option value="Round Robin">Round Robin</option><option value="Swiss">Swiss</option></select><select class="filter-sel" id="filter-tournaments-status" onchange="renderRes('tournaments')"><option value="">All Status</option><option value="Registration">Registration</option><option value="In Progress">In Progress</option><option value="Completed">Completed</option><option value="Cancelled">Cancelled</option></select></div>
<div class="count-label" id="count-tournaments"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Tournament Name</th><th>Game/Sport</th><th>Format</th><th>Date</th><th>Location</th><th>Max Participants</th><th>Status</th><th>Notes</th><th></th></tr></thead><tbody id="tbody-tournaments"></tbody></table></div></div><div class="tab-content" id="tab-participants">
<div class="stats" id="stats-participants"></div>
<div class="toolbar">
<input class="search" id="search-participants" placeholder="Search..." oninput="renderRes('participants')"><select class="filter-sel" id="filter-participants-status" onchange="renderRes('participants')"><option value="">All Status</option><option value="Registered">Registered</option><option value="Active">Active</option><option value="Eliminated">Eliminated</option><option value="Winner">Winner</option></select></div>
<div class="count-label" id="count-participants"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Tournament ID</th><th>Name</th><th>Email</th><th>Seed</th><th>Status</th><th></th></tr></thead><tbody id="tbody-participants"></tbody></table></div></div><div class="tab-content" id="tab-matches">
<div class="stats" id="stats-matches"></div>
<div class="toolbar">
<input class="search" id="search-matches" placeholder="Search..." oninput="renderRes('matches')"><select class="filter-sel" id="filter-matches-status" onchange="renderRes('matches')"><option value="">All Status</option><option value="Pending">Pending</option><option value="In Progress">In Progress</option><option value="Completed">Completed</option></select></div>
<div class="count-label" id="count-matches"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Tournament ID</th><th>Round</th><th>Match #</th><th>Player 1</th><th>Player 2</th><th>Score 1</th><th>Score 2</th><th>Winner</th><th>Status</th><th></th></tr></thead><tbody id="tbody-matches"></tbody></table></div></div></div><div class="modal-bg" id="mbg" onclick="if(event.target===this)closeModal()"><div class="modal" id="mdl"></div></div><script>var A="/api",data={},editId=null,activeRes="tournaments";var resCfg={"tournaments":{d:"Tournaments",f:[{n:"name",l:"Tournament Name",t:"text",r:true,o:[]},{n:"game",l:"Game/Sport",t:"text",r:false,o:[]},{n:"format",l:"Format",t:"select",r:false,o:["Single Elimination","Double Elimination","Round Robin","Swiss"]},{n:"date",l:"Date",t:"date",r:false,o:[]},{n:"location",l:"Location",t:"text",r:false,o:[]},{n:"max_participants",l:"Max Participants",t:"integer",r:false,o:[]},{n:"status",l:"Status",t:"select",r:false,o:["Registration","In Progress","Completed","Cancelled"]},{n:"notes",l:"Notes",t:"textarea",r:false,o:[]}]},"participants":{d:"Participants",f:[{n:"tournament_id",l:"Tournament ID",t:"text",r:true,o:[]},{n:"name",l:"Name",t:"text",r:true,o:[]},{n:"email",l:"Email",t:"email",r:false,o:[]},{n:"seed",l:"Seed",t:"integer",r:false,o:[]},{n:"status",l:"Status",t:"select",r:false,o:["Registered","Active","Eliminated","Winner"]}]},"matches":{d:"Matches",f:[{n:"tournament_id",l:"Tournament ID",t:"text",r:true,o:[]},{n:"round",l:"Round",t:"integer",r:false,o:[]},{n:"match_number",l:"Match #",t:"integer",r:false,o:[]},{n:"player1",l:"Player 1",t:"text",r:false,o:[]},{n:"player2",l:"Player 2",t:"text",r:false,o:[]},{n:"score1",l:"Score 1",t:"text",r:false,o:[]},{n:"score2",l:"Score 2",t:"text",r:false,o:[]},{n:"winner",l:"Winner",t:"text",r:false,o:[]},{n:"status",l:"Status",t:"select",r:false,o:["Pending","In Progress","Completed"]}]},};
...
['tournaments','participants','matches'].forEach(function(r){renderRes(r)});
}
}else{
